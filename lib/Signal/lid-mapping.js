...
export class LIDMappingStore {
	...
	async storeLIDPNMappings(pairs) {
		...
		for (const { lid, pn } of pairs) {
			if (!lid || !pn) {
				this.logger.warn(`Skipped invalid pair: lid=${lid}, pn=${pn}`);
				continue;
			}
			const isLid = isLidUser(lid) || isHostedLidUser(lid);
			const isPn = isPnUser(pn) || isHostedPnUser(pn);
			const isReverseLid = isLidUser(pn) || isHostedLidUser(pn);
			const isReversePn = isPnUser(lid) || isHostedPnUser(lid);
			if (!((isLid && isPn) || (isReverseLid && isReversePn))) {
			...
			validatedPairs.push({ 
				pnUser: isPn ? pnDecoded.user : lidDecoded.user, 
				lidUser: isLid ? lidDecoded.user : pnDecoded.user 
			});
		}
		...
		for (const { pnUser, lidUser } of validatedPairs) {
			...
			if (existingLidUser === lidUser) {
				this.logger.debug({ pnUser: pnUser, lidUser: lidUser }, 'LID mapping already exists, skipping');
				continue;
			}
			pairMap[pnUser] = lidUser;
		}
		if (Object.keys(pairMap).length === 0) return;
		this.logger.trace({ pairMap: pairMap }, `Storing ${Object.keys(pairMap).length} pn mappings`);
		const batchData = {};
