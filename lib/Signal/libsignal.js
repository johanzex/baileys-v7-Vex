...
/** Extract identity key from PreKeyWhisperMessage for identity change detection */ function extractIdentityFromPkmsg(ciphertext) {
	try {
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc, fetchDevices) {
	...
	const repository = {
		...
		async decryptMessage({ jid, type, ciphertext }) {
			...
			if (type === 'pkmsg') {
				...
				if (identityKey) {
					...
					if (identityChanged) {
						logger.info({ jid: jid, addr: addrStr }, 'Identity key changed or new contact, session will be re-established');
					}
				}
			}
				...
				while (true) {
					try {
						...
						if (err?.name === 'MessageCounterError') {
							if (retried) {
								logger.error({ jid, err }, 'MessageCounterError retry already attempted, throwing error');
								throw err;
							}
							retried = true;
							logger.warn({ jid }, 'MessageCounterError detected, deleting session and retrying');
							try {
								await repository.deleteSession([jid]);
								logger.info({ jid }, 'Session deleted, retrying decryption');
							} catch (deleteErr) {
								logger.error({ jid, deleteErr }, 'Failed to delete session during retry');
								throw deleteErr;
							}
						} else {
							throw err;
		...
		async migrateSession(fromJid, toJid) {
			...
			let deviceJids = [];
			if (fetchDevices) {
				deviceJids = await fetchDevices([fromJid]);
			} else {
