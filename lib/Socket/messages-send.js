...
import { groupMetadataCache as defaultCache } from '../Store/cache-factory.js';
import {
	areJidsSameUser,
	getBinaryNodeChild,
...
export const makeMessagesSocket = config => {
		...
		if (!media || forceGet || new Date().getTime() - media.fetchDate.getTime() > media.ttl * 1e3) {
			mediaConn = (async () => {
				...
				const node = {
					...
					authV2: mediaConnNode.attrs.auth_v2,
					maxBuckets: +mediaConnNode.attrs.max_buckets,
					fetchDate: new Date()
				};
	...
	const flushReceipts = async () => {
		...
		const toFlush = [...receiptQueue];
		receiptQueue.length = 0;
		for (const { jid, participant, messageIds, type } of toFlush) {
			await sendReceiptInternal(jid, participant, messageIds, type).catch(err => {
				logger.error({ err: err, jid: jid }, 'failed to flush receipt');
			});
		}
	};
	const sendReceiptInternal = async (jid, participant, messageIds, type) => {
	 ...
	 * */ const sendReceipt = async (jid, participant, messageIds, type) => {
		...
		if (type === 'read' || type === 'read-self') {
			receiptQueue.push({ jid, participant, messageIds, type });
			if (!receiptTimer) {
				receiptTimer = setTimeout(flushReceipts, 500);
			}
		} else {
			await sendReceiptInternal(jid, participant, messageIds, type);
		}
	};
	/** Correctly bulk send receipts to multiple chats, participants */ const sendReceipts = async (keys, type) => {
		const recps = aggregateMessageKeysNotFromMe(keys);
		for (const { jid, participant, messageIds } of recps) {
			await sendReceipt(jid, participant, messageIds, type);
		}
	};
	/** Bulk read messages. Keys can be from different chats & participants */ const readMessages = async keys => {
		const privacySettings = await fetchPrivacySettings();
		// based on privacy settings, we have to change the read type
		const readType = privacySettings.readreceipts === 'all' ? 'read' : 'read-self';
		await sendReceipts(keys, readType);
	};
	/** Fetch all the devices we've to send a message to */ const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
		const deviceResults = [];
		if (!useCache) {
		...
		if (result) {
			...
			if (lidResults.length > 0) {
				...
				try {
					...
					logger.warn({ e: e, count: lidResults.length }, 'failed to assert sessions for newly mapped LIDs');
				}
			}
	...
	const assertSessions = async (jids, force) => {
		...
		if (jidsRequiringFetch.length) {
			...
			logger.debug({ jidsRequiringFetch: jidsRequiringFetch, wireJids: wireJids }, 'fetching sessions');
			const result = await query({
				tag: 'iq',
	...
	const sendPeerMessage = async (message) => {
		...
		const msgId = await relayMessage(meJid, message, {
	...
	const sendPeerDataOperationMessage = async pdoMessage => {
		...
		return sendPeerMessage(protocolMessage);
	};
			...
			try {
				...
				if (dsmMessage) {
					...
					if (isOwnUser && !isExactSenderDevice) {
						...
						logger.debug({ jid: jid, targetUser: targetUser }, 'Using DSM for own device');
					}
				}
	...
	const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
		...
		await authState.keys.transaction(async () => {
			...
			if (isGroupOrStatus && !isRetryResend) {
				...
				if (isLid && meLid) {
					ownId = meLid;
					logger.debug({ to: jid, ownId: ownId }, 'Using LID identity for @lid conversation');
				} else {
					logger.debug({ to: jid, ownId: ownId }, 'Using PN identity for @s.whatsapp.net conversation');
				}
				...
				for (const { user, jid } of devices) {
					...
					if (isExactSenderDevice) {
						logger.debug({ jid: jid, meId: meId, meLid: meLid }, 'Skipping exact sender device (whatsmeow pattern)');
						continue;
					}
			...
			if (!isNewsletter && !isRetryResend && reportingMessage?.messageContextInfo?.messageSecret && shouldIncludeReportingToken(reportingMessage)) {
				try {
					...
					logger.warn({ jid: jid, trace: error?.stack }, 'failed to attach reporting token');
				}
			}
		...
		sendMessage: async (jid, content, options = {}) => {
			...
			if (Array.isArray(jid)) {
				...
				const fullMsg = await generateWAMessage('status@broadcast', content, {
					...
					...options,
					messageId: options.messageId || generateMessageID() || generateMessageIDV2(userJid)
				});
				for (const id of jid) {
					if (isJidGroup(id)) {
						try {
							const groupData = (cachedGroupMetadata ? await cachedGroupMetadata(id) : defaultCache.get(id)) || (await groupMetadata(id));
				...
				const fullMsg = await generateWAMessage(jid, content, {
					...
					cachedGroupMetadata: cachedGroupMetadata || (jid => defaultCache.get(jid)),
