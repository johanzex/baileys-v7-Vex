...
export const makeMessagesRecvSocket = (config) => {
  ...
  /** this mutex ensures that each retryRequest will wait for the previous one to finish */ const retryMutex =
    makeMutex();
  ...
  const handleMexNewsletterNotification = async (node) => {
    ...
    logger.info(
      { operation: operation, updates: updates },
    ...
    switch (child.tag) {
      ...
      case "update":
        ...
        if (settingsNode) {
          ...
          ev.emit("newsletter-settings.update", { id: from, update: update });
        }
        break;
      case "message":
  ...
  const initiateCall = async (jid, options = {}) => {
    ...
    const stanza = {
      ...
      content: [
        {
          tag: "offer",
          attrs: { "call-id": callId, "call-creator": meId, count: "0" },
          content: [
            { tag: isVideo ? "video" : "audio", attrs: {} },
            { tag: "net", attrs: {} },
            { tag: "encopt", attrs: { key: randomBytes(2).toString("hex") } },
    ...
    return { callId: callId, to: jid, isVideo: isVideo };
  };
  ...
  const handleEncryptNotification = async (node) => {
    ...
    const handleGroupNotification = async (fullNode, child, msg) => {
      const actingParticipantLid = jidNormalizedUser(fullNode.attrs.participant);
      const actingParticipantPn = fullNode.attrs.participant_pn ? jidNormalizedUser(fullNode.attrs.participant_pn) : undefined;
      
      if (actingParticipantLid && actingParticipantPn) {
        await signalRepository.lidMapping.storeLIDPNMappings([{ lid: actingParticipantLid, pn: actingParticipantPn }]);
      }
      
      const participantNode = getBinaryNodeChild(child, "participant");
      const affectedParticipantLid = participantNode?.attrs?.jid ? jidNormalizedUser(participantNode.attrs.jid) : actingParticipantLid;
      const affectedParticipantPn = participantNode?.attrs?.phone_number ? jidNormalizedUser(participantNode.attrs.phone_number) : actingParticipantPn;
      
      if (affectedParticipantLid && affectedParticipantPn) {
        await signalRepository.lidMapping.storeLIDPNMappings([{ lid: affectedParticipantLid, pn: affectedParticipantPn }]);
      }
    switch (child?.tag) {
      case "create":
  ...
  const handleDevicesNotification = async (node) => {
    ...
    if (
      areJidsSameUser(from, authState.creds.me.id) ||
    ...
    if (!devices || !devices.length || !devices[0]) {
      logger.debug({ from: from }, "no devices in notification, skipping");
      return;
    }
    ...
    const tag = child.tag;
    if (!deviceJid) {
      logger.debug({ tag: tag }, "no device jid in notification, skipping");
      return;
    }
    await devicesMutex.mutex(async () => {
      ...
      if (!existingCache.length) {
        logger.debug(
          { user: user, tag: tag },
          "device list not cached, skipping cache update",
        );
        return;
      }
      ...
      switch (tag) {
        case "add":
          logger.info({ deviceHash: deviceHash }, "device added");
          updatedDevices = [
          ...
          break;
        case "remove":
          logger.info({ deviceHash: deviceHash }, "device removed");
  ...
  const handleReceipt = async (node) => {
    ...
    try {
      ...
      await sendMessageAck(node).catch((ackErr) =>
        logger.error({ ackErr: ackErr }, "failed to ack receipt"),
      );
    }
  };
  const handleNotification = async (node) => {
    ...
    try {
      ...
      await sendMessageAck(node).catch((ackErr) =>
        logger.error({ ackErr: ackErr }, "failed to ack notification"),
      );
    }
  };
  const handleMessage = async (node) => {
    ...
    try {
      ...
      if (!!alt && (isLidUser(alt) || isPnUser(alt))) {
        const altServer = jidDecode(alt)?.server;
        const primaryJid = msg.key.participant || msg.key.remoteJid;
        if (!!primaryJid && (isLidUser(primaryJid) || isPnUser(primaryJid))) {
          if (altServer === "lid") {
        ...
        if (
          ...
          if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
            ...
            if (
              ...
              logger.debug(
                { msgId: msg.key.id, unavailableType: unavailableType },
            ...
            if (messageAge > PLACEHOLDER_MAX_AGE_SECONDS) {
              logger.debug(
                { msgId: msg.key.id, messageAge: messageAge },
            ...
            requestPlaceholderResend(cleanKey, msgData)
              .then((requestId) => {
                if (requestId && requestId !== "RESOLVED") {
                  logger.debug(
                    { msgId: msg.key.id, requestId: requestId },
  ...
  const handleCall = async (node) => {
    try {
      ...
      await sendMessageAck(node).catch((ackErr) =>
        logger.error({ ackErr: ackErr }, "failed to ack call"),
      );
    }
  };
